...
import { BookUpdateDTO } from '@/dto/books';
import { useAppDispatch, useAppSelector } from '@/hooks';
import { updateBookAction } from '@/store/books-slice/books-api-actions';
...
const validationSchema = Yup.object().shape({
  id: Yup.number().min(1, 'Обязательное поле.'),
  category_id: Yup.number().min(1, 'Обязательное поле.'),
  title: Yup.string().required('Обязательное поле.'),
  access: Yup.string().required('Обязательное поле.'),
  lang: Yup.string().required('Обязательное поле.'),
});

type BooksEditFormProps = {
  dto: BookUpdateDTO;
  setDTO: Dispatch<SetStateAction<BookUpdateDTO | null>>;
};

function BooksEditForm({
  dto,
  setDTO,
}: BooksEditFormProps): JSX.Element {
  const dispatch = useAppDispatch();
  const initialValues: BookUpdateDTO = dto;
  const categories = useAppSelector(getBookCategories);
  const fileRef = useRef<HTMLInputElement>(null);

  const onSubmit = async (
    values: BookUpdateDTO,
    helpers: FormikHelpers<BookUpdateDTO>,
  ) => {
    ...
    await dispatch(updateBookAction({
      id: values.id,
      formData,
      onSuccess: () => {
      ...
      {({ isSubmitting, setFieldValue }) => (
        <Form>
          <div className="flex items-center justify-between gap-2 mb-4">
            <h3 className="title">Редактирование книги</h3>
